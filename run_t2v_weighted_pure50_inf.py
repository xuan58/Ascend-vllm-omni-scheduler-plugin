...
def save_outputs(reqs: list[ReqResult], summary: dict[str, Any], out_dir: Path) -> None:
    ...
    ts = int(time.time())
    req_json = out_dir / f"t2v_weighted_pure50_inf_reqs_{ts}.json"
    req_csv = out_dir / f"t2v_weighted_pure50_inf_reqs_{ts}.csv"
    summary_json = out_dir / f"t2v_weighted_pure50_inf_summary_{ts}.json"
...
async def main() -> None:
    parser = argparse.ArgumentParser(description="Pure-image weighted T2V benchmark: 50 concurrent, rps=inf style.")
    ...
    # Weighted request mix (same as previous rounds)
    cases = [
    ...
    summary = {
        "round_name": "pure_image_weighted_50req_conc50_rps_inf",
